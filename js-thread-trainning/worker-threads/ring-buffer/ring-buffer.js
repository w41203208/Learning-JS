...
class SharedRingBuffer {
  constructor(shared) {
    ...
    this.ringBuffer = new RingBuffer(new Uint32Array(this.shared, 4, 3), new Uint8Array(this.shared, 16));
    this.lock = new Mutex(new Int32Array(this.shared, 0, 1), 0);
  }
  write(data) {
...
class RingBuffer {
  ...
  read(bytes) {
    ...
    if (this.head > this.tail || this.buffer.length - this.tail >= bytes) {
      ...
      fs.writeFileSync(1, `Read ${this.buffer.length} btyes\n`);
...
if (isMainThread) {
  ...
  if (isProducer) {
    ...
    while (true) {
      ...
      fs.writeFileSync(1, `Read ${readBytes.length} btyes\n`);
    }
  }
}
